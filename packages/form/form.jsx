...
var Input = createReactClass({
  ...
  render: function() {
    ...
    var input = (<input name={`${this.props.options.name}`} className="mdl-textfield__input"
      onChange={this.handleChange} value={value} type="text" id="sample3" />);
      if ('validation' in this.props.options) {
        errorSpan = (<span className="mdl-textfield__error">{this.props.options.validation.errorMsg}
        </span>);
        input = (<input name={`${this.props.options.name}`} className="mdl-textfield__input"
